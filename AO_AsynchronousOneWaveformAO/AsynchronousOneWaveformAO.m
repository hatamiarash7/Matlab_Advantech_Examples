...
%    5. Set the 'convertClkRate' to tell driver output data rate for 
%       each channel.

function AsynchronousOneWaveformAO()

% Make Automation.BDaq assembly visible to MATLAB.
BDaq = NET.addAssembly('Automation.BDaq');

% Define how many samples to makeup a waveform period.
...
try
    ...
    errorCode = GenerateWaveform(bufferedAoCtrl,...
        channelStart, channelCount, scaledWaveForm, ...
        scaledWaveForm.Length, H5T.enum_nameof(WaveStyle, int32(0)));
    if BioFailed(errorCode)    
        throw Exception();
    end
    ...
    % Step 6: Start Asynchronous One Waveform AO, 'Asynchronous' means
